...
async def get_max_app_keyboard(event, user_role="гость"):
    ...
    try:
        ...
        logger.error(f"Ошибка OpenAppButton: {e}")
...
async def send_response(bot_instance, chat_id, text, keyboard=None, remove_previous=True, user_id=None):
    """Отправка сообщений с управлением кэшем и удалением старых сообщений."""
    try:
        ...
        if remove_previous and cache_key in message_cache:
            try:
                ...
                logger.info(f"Удалили сообщение {old_message_id} в чате {chat_id} для {cache_key}")
            except Exception as e:
                logger.warning(f"Не удалось удалить сообщение {old_message_id}: {e}")
            finally:
        ...
        if keyboard:
            ...
            logger.info(f"Сохранили сообщение {sent_message.message_id} для {cache_key}")

        logger.info(f"Отправлено сообщение в чат {chat_id} для {cache_key}")
        return sent_message
    except Exception as e:
