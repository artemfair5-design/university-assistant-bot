...
async def send_response(bot_instance, chat_id, text, keyboard=None, remove_previous=True):
    ...
    try:
        ...
        if keyboard and hasattr(sent_message, 'id'):
            message_cache[chat_id] = sent_message.id
            logger.info(f"Сохранили сообщение с кнопками {sent_message.id} для чата {chat_id}")
